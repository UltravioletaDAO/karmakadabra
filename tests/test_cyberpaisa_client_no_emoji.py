...
async def test_cyberpaisa_purchases():
    ...
    print("\nã Step 1: Initialize cyberpaisa agent")
    print("-" * 80)
    ...
    try:
        ...
        print(f"   é Initial GLUE Balance: {initial_balance}")
    ...
    print("\nç Step 2: Discover karma-hello agent")
    print("-" * 80)
    ...
    try:
        ...
        if result.get("success"):
            ...
            if isinstance(data, dict):
                print(f"   ä Keys in response: {list(data.keys())[:5]}")

            tx_hash = result.get('tx_hash')
            if tx_hash:
                print(f"   ó Transaction: {tx_hash}")
                print(f"   ê View on Snowtrace: https://testnet.snowtrace.io/tx/{tx_hash}")
        else:
    ...
    print("\nç Step 4: Discover abracadabra agent")
    print("-" * 80)
    ...
    # Final balance
    print("\nä Step 6: Final Summary")
    print("-" * 80)

    final_balance = agent.get_balance()
    spent = initial_balance - final_balance

    print(f"é Initial Balance: {initial_balance} GLUE")
    print(f"é Final Balance:   {final_balance} GLUE")
