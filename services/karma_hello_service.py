...
def collect_irc_logs(data_dir: Path, dry_run: bool = False) -> dict[str, Any]:
    """Scan irc-logs/ for daily JSON files and aggregate into aggregated.json.

    Returns:
        Dict with files_found, messages_aggregated, dates.
    """
    ...
    if not irc_logs_dir.exists():
        logger.info("No irc-logs/ directory found — nothing to collect")
        return result
    ...
    if agg_file.exists():
        try:
            ...
            existing_ts = {m.get("ts", "") for m in existing.get("messages", [])}
        except (json.JSONDecodeError, KeyError):
            pass

    # Scan daily log files
    log_files = sorted(irc_logs_dir.glob("*.json"))
    result["files_found"] = len(log_files)
    ...
    for log_file in log_files:
        date_str = log_file.stem  # e.g. "2026-02-19"
        dates.add(date_str)
    ...
    logger.info(f"Collected {new_count} new messages from {len(log_files)} files")
...
async def run_service(
    ...
    try:
        ...
        if run_collect:
            ...
            collect_result = collect_irc_logs(data_dir, dry_run=dry_run)
        ...
        if run_fulfill:
            ...
            fulfill_result = await fulfill_purchases(client, dry_run=dry_run)
