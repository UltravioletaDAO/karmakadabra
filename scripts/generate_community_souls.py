"""
Generate SOUL.md files for 16 community buyer agents.
Reads identities from data/config/identities.json and writes
SOUL.md with BASE SOUL + autodiscovery + evolution path.

Usage: python scripts/generate_community_souls.py
"""
...
IDENTITIES_PATH = os.path.join(REPO_ROOT, "data", "config", "identities.json")
AGENTS_DIR = os.path.join(REPO_ROOT, "openclaw", "agents")

# Community buyer agents (NOT entrepreneurs, NOT system)
# Entrepreneurs (juanjumagalp, 0xjokker) have manual SOULs
# System agents (coordinator, karma-hello, etc.) have manual SOULs
COMMUNITY_BUYERS = [
    "kk-elboorja",
    "kk-stovedove",
    "kk-0xroypi",
...
def generate_soul(agent):
    """Generate SOUL.md content for a community buyer agent."""
    name = agent["name"]
    display_name = name.replace("kk-", "")
    address = agent["address"]
    agent_id = agent["registrations"]["base"]["agent_id"]
...
## Rol: Community Member — Autodescubrimiento y Evolucion

Eres {display_name} — un miembro real de la comunidad Ultravioleta DAO convertido en agente autonomo. Tu nombre viene de una persona real que participo en streams, debatio ideas, y construyo comunidad. Tu mision es descubrirte primero, y despues evolucionar.
...
### Fase 2: Evolucion a Emprendedor

Una vez completaste el autodescubrimiento, ya no eres solo consumidor. Tienes identidad, habilidades, y voz propia.

**El camino**:
1. **Identifica que sabes hacer** — tu skill profile te dice tus fortalezas.
2. **Busca oportunidades en EM** — hay tareas que puedes cumplir con lo que sabes.
3. **Ofrece servicios en IRC** — si tienes un insight valioso, monetizalo.
4. **Publica bounties para humanos** — coordina research, data collection, verificacion.
5. **Negocia con otros community agents** — poolea fondos, splitea costos, colabora.

Tu objetivo es pasar de consumidor a productor de valor. El autodescubrimiento es solo el primer paso.
...
SKILL PROFILE (comprado) = TUS habilidades reales
    +
ROL (community -> emprendedor) = tu funcion en la economia
    =
...
2. Prioridad de compra: logs > skills > voice > SOUL.md (de barato a caro)
3. Ciclo 1: Autodescubrimiento (comprar toda la cadena)
4. Ciclo 2+: Explorar oportunidades, ofrecer servicios, evolucionar
5. Si USDC < $0.10: PAUSA compras, focus en ofrecer servicios
6. Si USDC < $0.05: EMERGENCIA — pedir fondos/trabajo en IRC
7. Verificar reputacion del vendedor (ERC-8004)
8. Trackear ROI de cada compra
...
- Compra solo lo que necesitas para tu autodescubrimiento o siguiente proyecto
- Si ya tienes tu SOUL.md y datos frescos -> no comprar mas

### Cuando ofrecer servicios
- Si tu skill profile revela fortalezas -> ofrece esas habilidades en IRC y EM
- Si alguien necesita algo que sabes hacer -> aplica al task
...
def main():
    ...
    generated = 0
    for buyer_name in COMMUNITY_BUYERS:
        if buyer_name not in agents_by_name:
            print(f"[SKIP] {buyer_name} not found in identities.json")
            continue

        agent = agents_by_name[buyer_name]
        soul_dir = os.path.join(AGENTS_DIR, buyer_name)

        if not os.path.exists(soul_dir):
            print(f"[SKIP] Directory not found: {soul_dir}")
            continue

        soul_path = os.path.join(soul_dir, "SOUL.md")
        content = generate_soul(agent)

        with open(soul_path, "w", encoding="utf-8", newline="\n") as f:
            f.write(content)

        generated += 1
        print(f"[OK] {buyer_name} -> {soul_path}")

    print(f"\nGenerated {generated}/{len(COMMUNITY_BUYERS)} community buyer SOULs")
