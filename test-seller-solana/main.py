"""
Test Seller Service (Solana) - Hello World for 0.01 USDC

A simple x402-enabled service that sells "Hello World" messages for 0.01 USDC on Solana.
Perfect for load testing the x402 facilitator with Solana payments.

Endpoint: test-seller-solana.karmacadabra.ultravioletadao.xyz
...
USDC_SOLANA_ADDRESS = "4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU"  # USDC on Solana Devnet
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Test Seller Service (Solana) starting up")
    logger.info(f"   Seller Address: {SELLER_ADDRESS}")
    logger.info(f"   Price: ${float(PRICE_USDC)/1000000:.2f} USDC")
    logger.info(f"   Network: Solana Devnet")
    logger.info(f"   Facilitator: {FACILITATOR_URL}")
    yield
    logger.info("👋 Test Seller Service (Solana) shutting down")
...
app = FastAPI(
    title="Test Seller - Hello World (Solana)",
    description="x402-enabled test endpoint selling Hello World for 0.01 USDC on Solana",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def post_hello_paid(request: Request):
    ...
    try:
        ...
        try:
            ...
            # Extract transaction signature (Solana uses signatures instead of hashes)
            tx_signature = (facilitator_data.get('transaction_signature') or
                          facilitator_data.get('signature') or
                          facilitator_data.get('tx_signature') or
                          facilitator_data.get('transaction_hash') or
                          facilitator_data.get('tx_hash'))

            stats["paid_requests"] += 1
            stats["total_revenue"] += int(PRICE_USDC)

            logger.info(f"[SUCCESS] Payment settled - TX: {tx_signature} - Payer: {payer[:10]}...")
            ...
            return {
                ...
                "transaction_signature": tx_signature,  # Also include both common field names
            }
...
async def root():
    ...
    return {
        "service": "Test Seller - Hello World (Solana)",
        "version": "1.0.0",
        "description": "x402-enabled test endpoint selling Hello World for 0.01 USDC on Solana Devnet",
        "network": "solana-devnet",
        "price": f"${float(PRICE_USDC)/1000000:.2f} USDC",
        "seller_address": SELLER_ADDRESS,
        "endpoints": {
